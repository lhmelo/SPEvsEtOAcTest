---
title: "QCcleanup_RecoveryTest_Cyano_pH2"
date: July 26, 2017
...
datapath <- "~/Desktop/MetabolomicsProjects/SPEvsEtOAcTest/RecoveryTests/Cyano_pH2"
...
#Collect pH2 data
```{r}
data <- data %>% filter(grepl("Poo|pH2|Std|Blk", Replicate.Name))
```


#Load Internal Standard List
```{r}
setwd("~/Desktop/MetabolomicsProjects/SPEvsEtOAcTest/RecoveryTests/Cyano_pH2")
...
```{r}
setwd(datapath)
filename <- "QC_outputCyano_SPErecovery_pH2_PPL"
...
setwd(datapath)
  write.csv(Recovery, "RecoveryValues_postBMIS_pH2_Cyano.csv")
```
...
Recovery_raw <- temp.good %>%
  ...
  group_by(Compound.Name, SampID) %>%
  dplyr::summarise(mean.area.corrected = mean(area.corrected)) %>%
  mutate(SampID = gsub('SPEpH2-', '', SampID)) %>%
  spread(key = SampID, value=mean.area.corrected) %>%
  mutate(percent.recovery = (spikebefore/(spikeafter1*2))*100) %>%
  mutate(percent.recovery = round(percent.recovery)) %>%
  arrange(desc(percent.recovery)) %>%
...
setwd(datapath)
  write.csv(Recovery_raw, "RecoveryValues_rawareas_pH2_Cyano.csv")
```
...
setwd(datapath)
write_csv(data_nospike_conc, "BackgroundConcentrations_SPE_pH2_Cyano.csv")
