---
title: "QCcleanup_RecoveryTest_Cyano_pH5"
date: July 26, 2017
...
setwd(datapath)
filename <- "QC_outputCyano_SPErecovery_pH5_PPL"
...
```{r}
setwd("~/Desktop/MetabolomicsProjects/SPEvsEtOAcTest/RecoveryTests/Cyano_pH5")
...
```{r}
data_4hydroxybenz_allNormers <- data_normalized %>% 
  filter(Compound.Name == "4-hydroxybenzaldehyde") %>% 
  filter(!grepl("Std|Blk", Replicate.Name)) %>%
...
s <- ggplot(data_4hydroxybenz_allNormers, aes(x=Replicate.Name, y=Area)) + geom_bar(stat="identity") + facet_wrap( ~Normer, scales="free_y")+theme(axis.text.x = element_text(angle = 90, hjust = 1,vjust = 0.5), legend.position = "top")  + ggtitle("All IS Normalizations for 4-hydroxybenzaldehyde, wo blank, Cyano pH5")
```
...
setwd(datapath)
  write.csv(Recovery, "RecoveryValues_postBMIS_pH5_Cyano.csv")
```
...
setwd(datapath)
  write.csv(Recovery_raw, "RecoveryValues_rawareas_pH5_Cyano.csv")
```
...
setwd(datapath)
write_csv(data_nospike_conc, "BackgroundConcentrations_SPE_pH5_Cyano.csv")
