...
#Plot a single compound normalized to all IS
```{r}
...
#Calculate recoveries based on raw peak areas

```{r}
#Substract "no spike" avg value from treatment values

#Create a data.frame of average background values seen in the nospike samples
data_backgroundvalues_raw <- data_normalized %>%
  filter(grepl('nospike', Replicate.Name))%>%
  mutate(Compound.Name = as.factor(Compound.Name)) %>%
  group_by(Compound.Name) %>%
    dplyr::summarise(mean.background = mean(Area))

data_backgroundcorrected_raw <- data_normalized %>%
   filter(!grepl('nospike', Replicate.Name))

Unique.names <-unique(data_blankreplaced$Compound.Name)

Names <- colnames(data_backgroundcorrected_raw)
...
for (i in 1:length(Unique.names)) {
  ...
  data_backgroundvalue <- data_backgroundvalues_raw %>%
    filter(Compound.Name == Unique.names[i]) 
  
  data_backgroundvalue <- as.data.frame(data_backgroundvalue)
   data_backgroundcorrected_raw <- as.data.frame(data_backgroundcorrected_raw)
  
  Compound_set <- data_backgroundcorrected_raw %>% 
    filter(grepl('Smp', Replicate.Name)) %>%
    filter(!grepl("spikeafter_2", Replicate.Name)) %>%
    filter(!grepl("spikeafter_3", Replicate.Name)) %>%
    filter(Compound.Name == Unique.names[i]) %>%
...
Recovery_raw_trim <- Recovery_raw %>%
  select(Compound.Name, percent.recovery)


setwd(datapath)
  write.csv(Recovery_raw, "RecoveryValues_rawareas_pH8_Cyano.csv")
```
